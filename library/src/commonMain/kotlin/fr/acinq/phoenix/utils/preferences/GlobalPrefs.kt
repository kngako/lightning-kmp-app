...
package fr.acinq.phoenix.utils.preferences
...
import fr.acinq.phoenix.data.BaseWalletId
...
class GlobalPrefs(private val data: DataStore<Preferences>) {
    ...
    private companion object {
        ...
        private val LAST_USED_APP_CODE = stringPreferencesKey("LAST_USED_APP_CODE")
        private val SHOW_RELEASE_NOTES_SINCE = stringPreferencesKey("SHOW_RELEASE_NOTES_SINCE")
    }
...
//                log.error("could not deserialize available_wallets=$json: ", e)
    ...
    val getLastUsedAppCode: Flow<String?> = safeData.map { it[LAST_USED_APP_CODE] }
    suspend fun saveLastUsedAppCode(code: String) = data.edit { it[LAST_USED_APP_CODE] = code }
    ...
    val showReleaseNoteSinceCode: Flow<String?> = safeData.map { it[SHOW_RELEASE_NOTES_SINCE] }
    suspend fun saveShowReleaseNoteSinceCode(code: String?) = data.edit {
...
data class UserWalletMetadata(val walletId: WalletId, val name: String?, val avatar: String, val createdAt: Long?, val isHidden: Boolean) {
    @Composable
    fun nameOrDefault() = name?.takeIf { it.isNotBlank() } ?: "Default name"
}
