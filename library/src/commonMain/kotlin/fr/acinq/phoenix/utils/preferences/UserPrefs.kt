...
class UserPrefs(private val data: DataStore<Preferences>) {
    ...
    private val log = Logger.withTag("UserPrefs")
    ...
    val getBitcoinUnits: Flow<PreferredBitcoinUnits> = safeData.map {
        it[BITCOIN_UNITS]?.let {
            try {
                ...
                log.e("failed to decode bitcoin units: $it: ${e.message}")
                null
            }
    ...
    suspend fun saveBitcoinUnits(units: PreferredBitcoinUnits) = data.edit {
        try {
            ...
            log.e("failed to save bitcoin units list: $units: ${e.message}")
        }
    }
    ...
    val getFiatCurrencies: Flow<PreferredFiatCurrencies> = safeData.map {
        it[FIAT_CURRENCIES]?.let {
            try {
                ...
                log.e("failed to decode fiat currencies list: $it: ${e.message}")
                null
            }
    ...
    suspend fun saveFiatCurrencyList(preferredCurrencies: PreferredFiatCurrencies) = data.edit {
        try {
            ...
            log.e("failed to save fiat currencies list: ${e.message}")
        }
    }
    ...
    val getElectrumServer: Flow<ElectrumConfig.Custom?> = safeData.map {
        ...
        log.d("retrieved electrum address from datastore, host=$host port=$port key=$pinnedKey")
    ...
    suspend fun saveSwapAddressFormat(format: SwapAddressFormat) = data.edit {
        log.i("saving swap-address-format=$format")
    ...
    val getLiquidityPolicy: Flow<LiquidityPolicy> = safeData.map {
        try {
            ...
            log.e("failed to read liquidity-policy preference, replace with default: ${e.message}")
    ...
    suspend fun saveLiquidityPolicy(policy: LiquidityPolicy) = data.edit {
        log.i("saving new liquidity policy=$policy")
