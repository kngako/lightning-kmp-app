...
class EncryptedData(val version: Version, val data: ByteArray) {
    ...
    fun decrypt(keyManager: LocalKeyManager): ByteVector {
        ...
        val unzipped = Buffer().write(decryptedPayload).asInputStream().use { bis ->
    ...
    fun write(): ByteArray {
        return when (version) {
            Version.V1 -> {
                val bos = Buffer()
                bos.writeByte(version.code.toInt())
                bos.write(data)
                bos.readByteArray()
            }
        }
    ...
    companion object {
        ...
        fun encrypt(version: Version, data: ByteArray, keyManager: LocalKeyManager): EncryptedData {
            ...
            val payload = Buffer().run {
                ZipOutputStream(this.asOutputStream()).use { zos ->
        ...
        fun read(data: ByteArray): EncryptedData {
            return Buffer().write(data).asInputStream().use { bis ->
