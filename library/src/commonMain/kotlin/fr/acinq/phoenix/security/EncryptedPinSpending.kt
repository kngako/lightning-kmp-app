...
sealed class EncryptedPinSpending: EncryptedPin.KeystoreEncrypted() {
    ...
    companion object {
        const val SINGLE_WALLET_VERSION: Byte = 1
        const val MULTIPLE_WALLET_VERSION: Byte = 2

        fun encrypt(pinMap: Map<WalletId, String>): MultipleWallet {
            val json = Json.encodeToString(
                pinMap.map { (id, pin) -> id.nodeIdHash to pin }.toMap()
            )

            return keyStoreEncryption(KeyStoreNames.KEY_FOR_PINCODE_V1, json.encodeToByteArray()).let { cipherText ->
                MultipleWallet(
                    cipherText.first,
                    cipherText.second
                )
        ...
        fun deserialize(serialized: ByteArray): EncryptedPinSpending {
            ...
            when (val version = stream.readByte()) {
