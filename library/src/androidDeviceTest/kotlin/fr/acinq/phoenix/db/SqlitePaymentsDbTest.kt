 ...
 *
 * [AndroidJUnit4] delegates to the instrumentation runner on a device and to Robolectric on the host jvm, so
 * subclasses run unchanged in both `androidDeviceTest` and `androidHostTest`.
 */
...
class SqlitePaymentsDbTest : UsingContextTest() {
    ...
    /**
     * Keeps the migrated file around so that it can be pulled off the device, and inspected or diffed against
     * the ios run, afterwards. The process working directory is not writable here, so this goes to the
     * app-private external files directory instead.
     */
    private fun exportMigratedDb(fileName: String) {
        val context = getPlatformContext().applicationContext
        val destDir = File(context.getExternalFilesDir(null) ?: context.filesDir, "migration-v10-v11")
        destDir.mkdirs()
        context.getDatabasePath(fileName).copyTo(File(destDir, fileName), overwrite = true)
