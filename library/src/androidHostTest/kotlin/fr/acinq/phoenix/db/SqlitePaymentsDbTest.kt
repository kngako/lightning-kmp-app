...
import fr.acinq.phoenix.testLoggerFactory
...
/**
 * Base class for android tests that need a real `Context` and sample databases on disk.
 *
 * [AndroidJUnit4] delegates to Robolectric on the host jvm and to the instrumentation runner on a device, so
 * subclasses run unchanged in both `androidHostTest` and `androidDeviceTest`.
 */
@RunWith(AndroidJUnit4::class)
abstract class UsingContextTest {

    fun getPlatformContext(): PlatformContext = PlatformContext(ApplicationProvider.getApplicationContext())

    /**
     * Restores the given classpath resources -- packaged from `src/commonTest/resources` -- into the
     * app-private `databases` directory, which is where the sqldelight driver resolves a bare database name.
     * Any earlier copy is dropped along with its journal and wal side files, so that each test opens a
     * pristine, still unmigrated database.
     */
    fun setUpDatabase(context: PlatformContext, resourcePaths: List<String>) {
        val dbDir = context.applicationContext.getDatabasePath("test").parentFile
            ?: error("no databases directory for ${context.applicationContext.packageName}")
        dbDir.mkdirs()
        resourcePaths.forEach { resourcePath ->
            val fileName = resourcePath.substringAfterLast('/')
            dbDir.listFiles { file -> file.name.startsWith(fileName) }?.forEach { it.delete() }
            val resource = javaClass.classLoader?.getResourceAsStream(resourcePath)
                ?: error("sample database $resourcePath is not on the test classpath")
            resource.use { input -> File(dbDir, fileName).outputStream().use { input.copyTo(it) } }
        }
    }
}
...
class SqlitePaymentsDbTest : UsingContextTest() {
    ...
    @BeforeTest
    fun setupDatabases() {
        // `createSqlitePaymentsDb` builds a `SqliteContactsDb`, which is a `MainScope` and dispatches as soon as
        // it is constructed. Neither test runtime gives us a main dispatcher we can rely on, so install one.
        Dispatchers.setMain(mainThreadSurrogate)
        setUpDatabase(getPlatformContext(), sampleDbs)
    }
    ...
    /**
     * Opens [fileName] from the app-private databases directory -- which runs the migrations up to the current
     * schema -- and hands the result to [block], closing the driver afterwards.
     */
    private fun withMigratedDb(fileName: String, block: (SqlitePaymentsDb) -> Unit) {
        val driver = createPaymentsDbDriver(getPlatformContext(), fileName = fileName, onError = onError)
        try {
            block(createSqlitePaymentsDb(driver, metadataQueue = null, loggerFactory = testLoggerFactory))
        } finally {
            driver.close()
            exportMigratedDb(fileName)
        }
    }

    /**
     * Keeps the migrated file around so that it can be inspected, or diffed against the ios run, afterwards.
     *
     * This has to happen while the test that migrated the file is still running: Robolectric hands every test
     * method its own data directory, so a class-level sweep would only ever find an empty one.
     */
    private fun exportMigratedDb(fileName: String) {
        val destDir = File("build/test-results/migration-v10-v11/android")
        destDir.mkdirs()
        getPlatformContext().applicationContext.getDatabasePath(fileName)
            .copyTo(File(destDir, fileName), overwrite = true)
        println("copied migrated dbfile=$fileName to $destDir")
    }

    @Test
    fun read_v1_db() = runTest {
        withMigratedDb("payments-testnet-fedc36138a62ceadc8a93861d2c46f5ca5e8b418.sqlite") { paymentsDb ->
    ...
    @Test
    fun read_v1_db_additional() = runTest {
        withMigratedDb("payments-testnet-a224978853d2f4c94ac8e2dbb2acf8344e0146d0.sqlite") { paymentsDb ->
            val payments = paymentsDb.database.paymentsQueries.list(limit = Long.MAX_VALUE, offset = 0)
                .executeAsList()
                .map { Serialization.deserialize(it.data_).getOrThrow() }
    ...
    @Test
    fun read_v6_db() = runTest {
        withMigratedDb("payments-testnet-700486fc7a90d5922d6f993f2941ab9f9f1a9d85.sqlite") { paymentsDb ->
            val payments = paymentsDb.database.paymentsQueries.list(limit = Long.MAX_VALUE, offset = 0)
                .executeAsList()
                .map { Serialization.deserialize(it.data_).getOrThrow() }
    ...
    @Test
    fun read_v10_db_large_dataset() = runTest {
        withMigratedDb("payments-testnet-28903aff.sqlite") { paymentsDb ->
            val payments = paymentsDb.database.paymentsQueries.list(limit = Long.MAX_VALUE, offset = 0)
                .executeAsList()
                .map { Serialization.deserialize(it.data_).getOrThrow() }
    ...
    @Test
    fun read_v10_db_fees() = runTest {
        withMigratedDb("payments-testnet-f921bddf.sqlite") { paymentsDb ->
    ...
    @Test
    fun read_v10_db_liquidity() = runTest {
        withMigratedDb("payments-testnet-6a5e6f.sqlite") { paymentsDb ->
    ...
    companion object {
        /** Sample databases to restore before each test, as classpath resources. */
        private val sampleDbs = listOf(
            "sampledbs/paymentsdb/v1/payments-testnet-fedc36138a62ceadc8a93861d2c46f5ca5e8b418.sqlite",
            "sampledbs/paymentsdb/v1/payments-testnet-a224978853d2f4c94ac8e2dbb2acf8344e0146d0.sqlite",
            "sampledbs/paymentsdb/v6/payments-testnet-700486fc7a90d5922d6f993f2941ab9f9f1a9d85.sqlite",
            "sampledbs/paymentsdb/v10/payments-testnet-28903aff.sqlite",
            "sampledbs/paymentsdb/v10/payments-testnet-f921bddf.sqlite",
            "sampledbs/paymentsdb/v10/payments-testnet-6a5e6f.sqlite",
        )
    }
}
