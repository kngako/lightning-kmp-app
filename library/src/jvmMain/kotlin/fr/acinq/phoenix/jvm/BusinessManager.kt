// Ported from `fr/acinq/phoenix/ios/BusinessManager.kt`, which despite living in iosMain
// contains no ios API at all -- no platform.* imports, no cinterop, no NSObject. It is the
// closer of the two starting points: the android manager is built around an android
// Application it holds a reference to, while this one constructs its own PlatformContext,
// which is exactly what the jvm can do.
//
// It is a copy rather than a shared source set for the same reason ElectrumServersTest.jvm.kt
// is: sharing it means an intermediate source set that iosMain and jvmMain both depend on,
// which changes how the module is wired. Worth doing, separately.

package fr.acinq.phoenix.jvm
