...
import fr.acinq.phoenix.data.ChannelsWatcherOutcome
...
class ChannelsWatcher(context: Context, workerParams: WorkerParameters) : CoroutineWorker(context, workerParams) {
    ...
    private suspend fun watchWallet(walletId: WalletId, words: List<String>): Boolean {
        ...
        val dataStoreManager = DataStoreManager(
            ctx = PlatformContext(applicationContext),
            chain = chain,
        )
        ...
        if (res is StartBusinessResult.Failure) {
            ...
            internalPrefs.saveChannelsWatcherOutcome(ChannelsWatcherOutcome.Unknown(currentTimestampMillis()))
            return true
        }
        ...
        try {
            ...
            if (channelsAtBoot.isEmpty()) {
                ...
                internalPrefs.saveChannelsWatcherOutcome(ChannelsWatcherOutcome.Nominal(currentTimestampMillis()))
                return true
            } else {
            ...
            if (unknownRevokedAfterWatching.isNotEmpty()) {
                ...
                internalPrefs.saveChannelsWatcherOutcome(ChannelsWatcherOutcome.RevokedFound(currentTimestampMillis()))
    ...
    companion object {
        ...
        const val TAG = "fr.acinq.phoenix.android.services.ChannelsWatcher"
