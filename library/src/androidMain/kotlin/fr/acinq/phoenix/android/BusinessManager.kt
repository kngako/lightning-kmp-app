...
object BusinessManager {
    ...
    private val application by lazy { appContext as LightningApplication }
    ...
    suspend fun startNewBusiness(words: List<String>, isHeadless: Boolean): StartBusinessResult = startupMutex.withLock {
        ...
        val business = PhoenixBusiness(application.getPhoenixGlobal())
        ...
        return try {
            ...
            eventsMonitoringJobs[walletId] = BusinessMonitorJobs(
                monitorHeadlessPaymentsJob = if (isHeadless) {
                    scope.launch { monitorPaymentsWhenHeadless(walletId, walletMetadata, business.nodeParamsManager, application.getPhoenixGlobal().currencyManager, userPrefs) }
                } else null,
    ...
    private fun doStopBusiness(walletId: WalletId, running: BusinessRunning) {
        ...
        application.getPhoenixGlobal().currencyManager.stopMonitoringForWallet(walletId.nodeIdHash)
    }

    private suspend fun monitorFcmToken(business: PhoenixBusiness) {
        val token = application.getGlobalPrefs().getFcmToken.filterNotNull().first()
        ...
        combine(
            ...
            when (event) {
                is LiquidityEvents.Rejected -> {
                    ...
                    val walletMetadata = application.getGlobalPrefs().getAvailableWalletsMeta.first().getByWalletIdOrDefault(walletId)
